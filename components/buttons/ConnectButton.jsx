// ConnectButton.jsx — the hero of the whole brand. A 150x150 circular
// power button that transitions off -> connecting -> on. Self-contained:
// the power glyph is inlined from /Components/Btn-Main/Vector.svg (Figma).
const { useState } = React;

export function PowerIcon({ size = 52, color = "currentColor", style }) {
  return (
...
export function ConnectButton({ state = "off", size = 150, onClick }) {
  ...
  return (
    <button
      ...
      style={{
        width: size, height: size, borderRadius: "50%", border: 0, padding: 0,
        background: active ? "var(--brand-gradient)" : "var(--disabled)",
        cursor: "pointer", position: "relative",
        boxShadow: active ? "0 12px 32px var(--brand-glow)" : "none",
        display: "flex", alignItems: "center", justifyContent: "center",
        transition: "transform var(--dur-base) var(--ease-ios), background var(--dur-base)",
        transform: pressed ? "scale(0.97)" : "scale(1)",
        opacity: isConn ? 0.92 : 1,
      }}
    >
      <PowerIcon size={size * 0.35} color={active ? "#fff" : "var(--fg-3)"} />
      {isConn && (
        <span style={{
      ...
      <style>{`@keyframes vpnPulse{0%,100%{transform:scale(1);opacity:.55}50%{transform:scale(1.06);opacity:0}}`}</style>
    </button>
  );
}
